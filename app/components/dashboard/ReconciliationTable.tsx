...
const ReconciliationTable = ({ clientId, isAdmin = false }: { clientId?: string; isAdmin?: boolean }) => {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      ...
      q = query(collection(db, "transactions"), where("clientId", "==", clientId), orderBy("createdAt", "desc"));
    }
